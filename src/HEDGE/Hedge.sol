...
contract Hedge is BaseHook {
    ...
    mapping(Currency => mapping(uint256 => Trigger[]))
        public triggersByCurrency;
    ...
    function afterSwap(
        ...
        uint256 numerator1 = uint256(params.sqrtPriceLimitX96) *
            uint256(params.sqrtPriceLimitX96);
        uint256 numerator2 = 10 ** 18;
        uint256 currency0Price = FullMath.mulDiv(
            numerator1,
            numerator2,
            1 << 192
        );
        ...
        for (uint256 i = 0; i < prices.length; i++) {
            Trigger[] memory triggers = triggersByCurrency[key.currency0][
                prices[i]
            ];
    ...
    function lockAcquiredUnwind(
        ...
        if (balance >= trigger.unwindAmount) {
            token.transferFrom(
                trigger.owner,
                address(poolManager),
                trigger.unwindAmount
            );
            poolManager.settle(trigger.currency1);
            BalanceDelta delta = poolManager.swap(key, params, abi.encode(""));

            uint256 token0Amount = uint256(uint128(delta.amount0()));
            poolManager.safeTransferFrom(
                address(this),
                address(poolManager),
                uint256(uint160(Currency.unwrap(trigger.currency0))),
                token0Amount,
                ""
            );
    ...
    function lockAcquiredHedge(
        PoolKey calldata key,
        IPoolManager.SwapParams calldata params,
        Trigger memory trigger
    ) external selfOnly returns (uint256 token1Amount) {
        ...
        if (balance >= trigger.maxAmountSwap) {
            ...
            poolManager.safeTransferFrom(
                address(this),
                address(poolManager),
                uint256(uint160(Currency.unwrap(trigger.currency1))),
                token1Amount,
                ""
            );
