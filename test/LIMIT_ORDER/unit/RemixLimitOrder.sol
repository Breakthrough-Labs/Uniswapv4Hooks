...
import {LimitOrder} from "../../../src/LIMIT_ORDER/LimitOrder.sol";
...
contract LimitiOrderTest {
    ...
    function setUp() public {
        ...
        hook = new LimitOrder{salt: salt}(IPoolManager(address(manager)));
    ...
    function testGetTickLowerLast() public {
        if (hook.getTickLowerLast(poolId) == 0) {
            emit TestLog("TickLowerLast test", "PASSED: TickLowerLast is 0");
        } else {
            emit Failure("FAILED: TickLowerLast is not 0");
        }
    }
